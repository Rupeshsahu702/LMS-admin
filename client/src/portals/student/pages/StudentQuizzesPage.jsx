...
import { ClipboardList, Loader2, AlertCircle, BookOpen } from 'lucide-react';

import LearningCard from '../components/LearningCard';
import { useQuizzesByCourse } from '../hooks';

const StudentQuizzesPage = () => {
  const { courses, loading, error, refetch } = useQuizzesByCourse();
  ...
  if (!courses || courses.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-black text-white">
        ...
        <p className="text-zinc-400">No quizzes available</p>
        <p className="text-zinc-500 text-sm mt-2">Enroll in courses to access quizzes</p>
      </div>
    );
  }
  ...
  const formattedCourses = courses.map(course => ({
    ...
    type: 'Quizzes',
    total: course.totalQuizzes,
    completed: course.completedQuizzes,
